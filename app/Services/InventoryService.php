...
class InventoryService
{
    ...
    {
        if ($from->id === $to->id) throw ValidationException::withMessages(['to_store_id' => 'Tujuan transfer harus berbeda dari toko asal.']);
        if ($quantity <= 0) throw ValidationException::withMessages(['quantity' => 'Jumlah transfer harus lebih dari 0.']);
        ...
        DB::transaction(function () use ($user, $from, $to, $product, $quantity, $reason): void {
            ...
            $firstInventory = $this->lockInventory($first, $product);
            $secondInventory = $this->lockInventory($second, $product);
            $source = $from->id === $first->id ? $firstInventory : $secondInventory;
            $destination = $to->id === $first->id ? $firstInventory : $secondInventory;
    ...
    private function change(User $user, Store $store, Product $product, float $quantity, string $type, ?float $unitCost, ?string $reason): Inventory
    {
        return DB::transaction(function () use ($user, $store, $product, $quantity, $type, $unitCost, $reason): Inventory {
            if ($quantity <= 0) throw ValidationException::withMessages(['quantity' => 'Jumlah harus lebih dari 0.']);
            $inventory = $this->lockInventory($store, $product);
            $before = (float) $inventory->quantity;
    ...
    {
        Inventory::query()->firstOrCreate(['store_id' => $store->id, 'product_id' => $product->id], ['quantity' => 0, 'reserved_quantity' => 0]);
        return Inventory::query()->where('store_id', $store->id)->where('product_id', $product->id)->lockForUpdate()->firstOrFail();
    }

    private function apply(Inventory $inventory, float $delta): void
    {
        $newQuantity = (float) $inventory->quantity + $delta;
        if ($newQuantity < 0) throw ValidationException::withMessages(['quantity' => 'Stok tidak boleh menjadi negatif.']);
