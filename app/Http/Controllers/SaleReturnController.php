...
class SaleReturnController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $query = SaleReturn::query()->with(['sale', 'store', 'processor'])->whereIn('store_id', $stores->pluck('id'))->latest('returned_at');
        if ($request->filled('store_id')) { $storeId = (int) $request->input('store_id'); $this->authorizeStore($request, $storeId); $query->where('store_id', $storeId); }
        if ($request->filled('search')) { $search = trim($request->input('search')); $query->where(fn ($q) => $q->where('return_number', 'ilike', "%{$search}%")->orWhereHas('sale', fn ($sale) => $sale->where('invoice_number', 'ilike', "%{$search}%"))); }
        return view('sale-returns.index', ['returns' => $query->paginate(15)->withQueryString(), 'stores' => $stores]);
    }
    ...
    public function create(Request $request, Sale $sale): View
    {
        ...
        $returned = $sale->returns->where('status', 'completed')->flatMap(fn ($return) => $return->items)->groupBy('sale_item_id')->map(fn ($items) => $items->sum(fn ($item) => (float) $item->quantity));
    ...
    {
        $user = $request->user();
        if ($user->hasRole(['super_admin', 'owner'])) return Store::query()->where('is_active', true)->orderBy('name')->get();
